...
public class ShowCarbonGuide5Procedure {
	@SubscribeEvent
	...
	private static boolean execute(@Nullable Event event, LevelAccessor world) {
		if (SustanabilityProjectModVariables.MapVariables.get(world).waterlevel <= 900
				&& SustanabilityProjectModVariables.MapVariables.get(world).waterlevel > 601) {
			return true;
		}
		return false;
	}
