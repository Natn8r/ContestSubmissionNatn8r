...
public class ShowCarbonGuide4Procedure {
	@SubscribeEvent
	...
	private static boolean execute(@Nullable Event event, LevelAccessor world) {
		if (SustanabilityProjectModVariables.MapVariables.get(world).waterlevel <= 600
				&& SustanabilityProjectModVariables.MapVariables.get(world).waterlevel > 401) {
			return true;
		}
		return false;
	}
