...
public class ShowCarbonGuide1Procedure {
	@SubscribeEvent
	...
	private static boolean execute(@Nullable Event event, LevelAccessor world) {
		if (SustanabilityProjectModVariables.MapVariables.get(world).waterlevel <= 200
				&& SustanabilityProjectModVariables.MapVariables.get(world).waterlevel > 0) {
			return true;
		}
		return false;
	}
