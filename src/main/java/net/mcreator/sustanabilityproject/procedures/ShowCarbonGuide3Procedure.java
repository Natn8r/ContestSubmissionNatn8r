...
public class ShowCarbonGuide3Procedure {
	@SubscribeEvent
	...
	private static boolean execute(@Nullable Event event, LevelAccessor world) {
		if (SustanabilityProjectModVariables.MapVariables.get(world).waterlevel <= 400
				&& SustanabilityProjectModVariables.MapVariables.get(world).waterlevel > 300) {
			return true;
		}
		return false;
	}
